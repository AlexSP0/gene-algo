...
BobTheResercher::BobTheResercher(
    double crossRate, double mutateRate, int popSize, int numBits, int geneLen)
{
...
void BobTheResercher::crossover(const std::vector<int> &mum,
...
void BobTheResercher::breedNewGeneration(int newGenerationSize)
{
    std::vector<Genome> newGeneration;

    for (size_t i = 0; i < newGenerationSize / 2; i++)
    {
    ...
    this->populationSize = newGeneration.size();
    populationGenoms     = newGeneration;
    generationNumber++;
...
int BobTheResercher::getPopulationSize()
{
